...
namespace Arua_content_importer.Common.FileHandler
{
	public class STL
	{
		...
		{
			int entryIndex = this.GetEntryIndex(string_ID);
			if (entryIndex != -1)
			{
				for (int i = 0; i < this.languageCount; i++)
				{
					this.entry[entryIndex].text[i] = text;
					this.entry[entryIndex].comment[i] = "";
					this.entry[entryIndex].quest1[i] = "";
					this.entry[entryIndex].quest2[i] = "";
				}
			}
			else
			{
		...
		public void AddEntry(int ID, string string_ID, string text, string comment)
		{
			...
			if (entryIndex != -1)
			{
				...
				{
					this.entry[entryIndex].text[i] = text;
					this.entry[entryIndex].comment[i] = comment;
					this.entry[entryIndex].quest1[i] = "";
					this.entry[entryIndex].quest2[i] = "";
				}
			}
		...
		public void AddEntry(int ID, string string_ID, string text, string comment, string quest1, string quest2)
		{
			...
			if (entryIndex != -1)
			{
				for (int i = 0; i < this.languageCount; i++)
				{
					this.entry[entryIndex].text[i] = text;
					this.entry[entryIndex].comment[i] = comment;
					this.entry[entryIndex].quest1[i] = quest1;
					this.entry[entryIndex].quest2[i] = quest2;
				}
			}
			else
			{
		...
		public int GetEntryIndex(string string_ID)
		{
			int num = 0;
			while (num < this.entry.Length && !(this.entry[num].string_ID == string_ID))
			{
				num++;
			}
			return (num == this.entry.Length) ? -1 : num;
		}
	}
}
